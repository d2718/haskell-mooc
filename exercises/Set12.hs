...
fmap2 f xs = fmap (fmap f) xs

fmap3 :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 f xs = fmap (fmap2 f) xs
...
instance Functor List where
  fmap f Empty         = Empty
  fmap f (LNode x cdr) = LNode (f x) (fmap f cdr)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode x y cdr) = (TwoNode (f x) (f y) (fmap f cdr))
...
count t f = foldr (\ x accum -> if x == t then accum + 1 else accum)
                  0 f
...
inBoth f g = foldr (\ x accum -> if elem x g then x:accum else accum)
                   [] f
...
instance Foldable List where
  foldr f initial Empty = initial
  foldr f initial (LNode val cdr) = f val (foldr f initial cdr)
...
instance Foldable TwoList where
  foldr f initial TwoEmpty = initial
  foldr f initial (TwoNode x y cdr) =
    f x (f y (foldr f initial cdr))
