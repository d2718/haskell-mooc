...
visit maze place = do
  ...
  if elem place been
  then return ()
  else do put (place:been)
          case (lookup place maze) of
            Just rooms -> mapM_ (visit maze) rooms
            Nothing -> return ()
...
path maze place1 place2 =
  let (_, places) = runState (visit maze place1) []
  in elem place2 places
...
findSum2 ks []     = []
findSum2 ks (n:ns) = findSums ks n ++ findSum2 ks ns
  where findSums values n =
          do  a <- ks
              b <- ks
              if (a + b == n)
              then [(a, b, n)]
...
allSums []  = [0]
allSums [x] = [x, 0]
allSums (x:cdr) = map (\y -> x + y) (allSums cdr) ++ allSums cdr
