...
firstHalf xs = if even (length xs)
               ...
               else take (1 + (div (length xs) 2)) xs
...
capitalize text = unwords (map capitalizeFirst (words text))

capitalizeFirst :: String -> String
capitalizeFirst [] = []
capitalizeFirst (x:rest) = (toUpper x) : rest
...
powers' k max accum =
  if head accum > max
    then reverse (tail accum)
    else powers' k max ((k * head accum) : accum)
------------------------------------------------------------------------------
...
joinToLength len chunks = 
  filter (\str -> length str == len) [x ++ y | x <- chunks, y <- chunks ]
