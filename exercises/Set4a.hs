...
rangeOf list = (sup list) - (inf list)
  where sup (x:[]) = x
...
longest :: Ord a => [[a]] -> [a]
longest (x:[]) = x
longest (x:rest) = longestCmp x (longest rest)
  where longestCmp xs ys =
          case signum (length xs - length ys) of 
            -1 -> ys
            1  -> xs
            0  -> if head xs < head ys then xs else ys
...
freqs xs = foldr insertIn Map.empty xs
  where insertIn :: (Eq a, Ord a) => a -> Map.Map a Int -> Map.Map a Int
        insertIn x m =
          case Map.lookup x m of
            Just y  -> Map.insert x (y + 1) m
