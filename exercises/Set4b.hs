...
countHelper (Just a) count = count
...
maxHelper n max = if n > max then n else max
...
slHelper nextVal (curSum, curLen) = (curSum + nextVal, curLen + 1)
...
concatHelper :: [a] -> [a] -> [a]
concatHelper next accum = next ++ accum
...
largestHelper next accum
  | next > head accum  = [next]
  | next == head accum = (next:accum)
  | otherwise          = accum
