...
buyOneMore (MkShoppingEntry name price qty) = MkShoppingEntry name price (qty + 1)
...
rgb col = case col of
  ...
  Blue  -> [0.0, 0.0, 1.0]
  Mix col0 col1 ->
    map (/2) (zipWith (+) (rgb col0) (rgb col1))
  Invert col0 -> map (1 - ) (rgb col0)
...
prettyPrint b = reverse (subPrint b)
  where subPrint End   = []
        subPrint (O b) = '0' : subPrint b
        subPrint (I b) = '1' : subPrint b
