...
allValues condition (Node val left right) =
  if allValues condition left && allValues condition right
  then condition val
  else False
...
cull val (Node x left right) =
  if x == val
  then Empty
  else (Node x (cull val left) (cull val right))
...
isOrdered (Node val Empty (Node x l r)) = val < x && isOrdered (Node x l r)
isOrdered (Node val (Node x l r) Empty) = x < val && isOrdered (Node x l r)
isOrdered (Node val (Node x l r) (Node y w z)) =
  x < val && val < y && isOrdered (Node x l r) && isOrdered (Node y w z)
isOrdered _ = True
...
walk (StepL:rest) (Node _ left _)  = walk rest left
walk (StepR:rest) (Node _ _ right) = walk rest right
...
set (StepL:rest) val (Node old left right) = Node old (set rest val left) right
set (StepR:rest) val (Node old left right) = Node old left  (set rest val right)
...
search val (Node x left right) =
  if x == val
  then Just []
  else case ((search val left), (search val right)) of
        ((Just list), _) -> Just (StepL:list)
        (_, (Just list)) -> Just (StepR:list)
        _                -> Nothing
