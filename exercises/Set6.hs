...
instance Eq Name where
  Name left == Name right =
    ...
    then foldr chk True (zip left right)
    else False
    where chk :: (Char, Char) -> Bool -> Bool
          chk _ False = False
          chk (lch, rch) True = Data.Char.toLower lch == Data.Char.toLower rch
...
instance Eq a => Eq (List a) where
  Empty == Empty = True
  LNode lval left == LNode rval right =
    if lval == rval
    then left == right
    else False
  _ == _ = False
...
instance Eq RationalNumber where
  RationalNumber ln ld == RationalNumber rn rd = ln * rd == ld * rn
...
simplify (RationalNumber n d) =
  let gf = gcd n d
  in RationalNumber (div n gf) (div d gf)
...
instance Num RationalNumber where
  RationalNumber ln ld + RationalNumber rn rd = 
    simplify (RationalNumber ((ln * rd) + (rn * ld)) (ld * rd)) 
  RationalNumber ln ld * RationalNumber rn rd =
    simplify (RationalNumber (ln * rn) (ld * rd))
  abs (RationalNumber n d) = RationalNumber (abs n) (abs d)
  signum (RationalNumber n d) = fromInteger (signum (n * d))
  fromInteger x = RationalNumber x 1
  negate (RationalNumber n d) = RationalNumber (0 - n) d
