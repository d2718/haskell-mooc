...
prettyPrint dim queens = f (1, 1) (sort queens)
  where
    f :: Coord -> [Coord] -> String
    f (y, x) qs
      | y > dim = []
      | x > dim = '\n' : f (nextRow (y, x)) qs
      | otherwise = case qs of
          coords:rest -> if coords == (y, x)
                         then 'Q' : f (nextCol (y, x)) rest
                         else '.' : f (nextCol (y, x)) (coords:rest)
          _           -> '.' : f (nextCol (y, x)) []
...
danger candidate stack = any oneThreat stack
    where oneThreat :: Coord -> Bool
          oneThreat queen = any (\ f -> f candidate queen) fs
          fs = [sameRow, sameCol, sameDiag, sameAntidiag]
...
prettyPrint2 dim queens = f (1, 1) (sort queens)
  where
    f :: Coord -> [Coord] -> String
    f (y, x) qs
      | y > dim = []
      | x > dim = '\n' : f (nextRow (y, x)) qs
      | otherwise = ch : f (nextCol (y, x)) left
          where
            (ch, left) =
              case qs of
                coords:rest -> if coords == (y, x)
                               then ('Q', rest)
                               else if danger (y, x) queens
                                    then ('#', qs)
                                    else ('.', qs)
                _           -> if danger (y, x) queens
                               then ('#', qs)
                               else ('.', qs)
...
fixFirst n ((y, x):rest)
  | x > n              = Nothing
  | danger (y, x) rest = fixFirst n ((nextCol (y, x)):rest)
  | otherwise          = Just ((y, x):rest)
...
backtrack (_:rest) = (nextCol (head rest)):(tail rest)
...
finish n s =
  let newStack = step n s
  in  if length newStack - 1 == n
      then tail newStack
      else finish n newStack
...
showSoln n = do
  putStrLn $ prettyPrint n (solve n)